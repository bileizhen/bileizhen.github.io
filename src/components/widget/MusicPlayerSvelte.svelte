...
const svelteConfig = config.svelte;

// 音乐播放器模式，可选 "local" 或 "meting"
let mode = svelteConfig?.meting_api ? "meting" : "local";
// Meting API 地址
let meting_api = svelteConfig?.meting_api;
// Meting API 的 ID
let meting_id = svelteConfig?.id;
// Meting API 的服务器
let meting_server = svelteConfig?.server ?? "netease";
// Meting API 的类型
let meting_type = svelteConfig?.type ?? "playlist";
